...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    def build_optimizer_group_ranges(cls, param_groups, model_gbuf_ranges):
        ...
        for group_index, group in enumerate(param_groups):
            for param in group["params"]:
                ensure_valid(param.requires_grad, "param.requires_grad is None")
    ...
    def save_parameter_state(self, filename):
        ...
        data_parallel_global_ranks = list(parallel_state._DATA_PARALLEL_GLOBAL_RANKS)
        ...
        for model_idx, gbuf_range_maps in enumerate(self.model_gbuf_ranges):
            ...
            dtype_state = {}
            ensure_valid(len(gbuf_range_maps) == 1, "single dtype supported, for now.")
    ...
    def load_parameter_state(self, filename, iteration):
        ...
        for model_idx, gbuf_range_maps in enumerate(self.model_gbuf_ranges):
            for dtype, gbuf_range_map in gbuf_range_maps.items():
                ...
                for model_param, param_range_map in \
                    ...
                    optim_state['step'] = iteration
                    # print(f"=================== {main_param} {optim_state}")
